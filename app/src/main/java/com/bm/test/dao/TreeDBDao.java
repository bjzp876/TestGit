...
public class TreeDBDao {
    ...
    public long insertData(Tree tree) {
        ...
        values.put(KEY_PHONE, tree.getPhone());
    ...
    public List<Tree> queryData(long id) {
        Cursor results = mDatabase.query(TABLE_NAME, new String[]{KEY_ID, KEY_NAME, KEY_PHONE, KEY_PRICE},
    ...
    //根据手机号查询
    public List<Tree> queryPhoneList(String phone) {
        Cursor results = mDatabase.query(TABLE_NAME, new String[]{KEY_ID, KEY_NAME, KEY_PHONE, KEY_PRICE},
                KEY_PHONE + "=" + phone, null, null, null, null);
        return convertToTree(results);

    }

    //根据手机号查询
    public List<Tree> queryNameList(String name) {
        Cursor results = mDatabase.query(TABLE_NAME, new String[]{KEY_ID, KEY_NAME, KEY_PHONE, KEY_PRICE},
    ...
    private static class TreeDBOpenHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            try {
                final String sqlStr = "create table if not exists " + TABLE_NAME + " (" + KEY_ID + " integer primary key autoincrement, " + KEY_NAME + " text not null, " + KEY_PHONE + " string," + KEY_PRICE + " float);";
                db.execSQL(sqlStr);
